...
impl Byte {
    pub fn decode_take<'de>(
        ...
        match self {
            ...
            Self::Huffman { alphabet, bit_lens } => {
                if alphabet.len() == 1 {
                    // Single-symbol Huffman: all bytes have the same value.
                    let byte = alphabet[0] as u8;
                    let buf = vec![byte; len];
                    Ok(external_data_readers.alloc_owned_buf(buf))
                } else {
                    // Multi-symbol Huffman: decode each byte individually.
                    let decoder = CanonicalHuffmanDecoder::new(alphabet, bit_lens);
                    let mut buf = Vec::with_capacity(len);
                    for _ in 0..len {
                        let value = decoder.decode(_core_data_reader).map(|i| i as u8)?;
                        buf.push(value);
                    }
                    Ok(external_data_readers.alloc_owned_buf(buf))
                }
    ...
    pub fn encode_extend(
        ...
        match self {
            ...
            Self::Huffman { .. } => Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "Huffman byte encoding: encode_extend not implemented",
            )),
        }
    }
}
...
impl Encode<'_> for Byte {
    ...
    fn encode(
        ...
        match self {
            ...
            _ => Err(io::Error::new(
                io::ErrorKind::Unsupported,
                format!("encode_byte not implemented for: {:?}", self),
            )),
        }
    }
}
