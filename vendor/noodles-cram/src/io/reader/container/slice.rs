...
fn get_slice_reference_sequence<'c>(
    ...
    if external_reference_sequence_is_required {
        ...
        // Try to get the reference sequence from the repository. If the
        // repository is empty (e.g., browser path without a FASTA), skip
        // the reference — alignment positions are still available without
        // decoded sequences. This patch enables CRAM reading without a
        // reference FASTA for position-only queries.
        ...
        if let Some(sequence) = sequence {
            ...
            // No reference available — return None. Records can still be
            // iterated for alignment positions; only sequence decoding fails.
            Ok(None)
        }
...
fn get_record_reference_sequence<'c>(
    ...
    match sequence {
        Some(sequence) => Ok(Some(ReferenceSequence::External { sequence })),
        // Reference not loaded for this chromosome — skip sequence
        // decoding. Record positions are still available.
        None => Ok(None),
    }
}
