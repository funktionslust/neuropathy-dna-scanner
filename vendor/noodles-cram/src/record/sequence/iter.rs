        ...
        loop {
            match self.state {
                State::Next => {
                        ...
                        self.features.next()
                    {
                        let bases =
                            if let Some(reference_sequence) = self.reference_sequence.as_ref() {
                                ...
                                // No reference available — emit placeholder bases
                                // instead of panicking. This allows position-only
                                // iteration without a reference FASTA.
                                &[]
                            } else {
                                &[]
                            };
                    ...
                    } else if usize::from(self.last_read_position) <= self.read_length {
                        // No reference available — emit N for this position
                        // and advance. This will re-enter State::Next on the
                        // next call, emitting one N per remaining position
                        // until last_read_position == read_length + 1.
                        self.last_read_position = self
                            .last_read_position
                            .checked_add(1)
                    ...
                    self.state = match feature {
                        ...
                        Feature::Substitution { code, .. } => {
                            if let Some(reference_sequence) = self.reference_sequence.as_ref() {
                                ...
                                // No reference — emit N for substitutions.
                                State::Base(b'N')
                            }
                        }
